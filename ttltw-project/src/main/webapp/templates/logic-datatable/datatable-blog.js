        ...
        columnDefs: [
            {
                ...
                render:function (data,type,row,meta) {
                    if (data ==1) {
                        return '<span class ="badge bg-warning">Hiển thị</span> ';
                    }else if(data==0){
                        return '<span class ="badge bg-info">Bị ẩn</span> ';
                    }
                }
            },
            {
                ...
                render: function (data, type, row, meta) {
                    let allButton = '';
                    const updateBtn = `<a href="http://localhost:8080/admin/blog-management/editing?id=${row.id}" data-bs-toggle="tooltip" title="Chỉnh sửa bài viết "
                       class="edit">
